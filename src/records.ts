...
// v5: 기록을 빈 화면이 아니라 입력 양식(record-template.ts) 위에 쓰게 되면서, 채워지지 않은
...
# 빈칸 처리 (기록은 미리 채워진 입력 양식 위에 작성됩니다)
...
- 와드에 파트가 여러 개인데 기록이 일부 파트만 다루고 있어, 나머지 파트를 수행했는지 알 수 없다.
  단, 입력 양식의 항목이 빈칸으로 남아 그 파트가 빠진 경우는 해당하지 않는다(빈칸은 의도적으로 비운 것으로 본다).
...
export async function parseWodRecord(
  ...
  // 파트가 0개인 것은 오류가 아니다. 입력 양식을 한 칸도 채우지 않고 저장하면 담을 파트가 없는 게
